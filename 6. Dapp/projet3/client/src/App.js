...
class App extends Component {
  ...
  componentDidMount = async () => {
    try {
      ...
      const owned = false;
      if (owner === accounts[0]) { let owned = true; }
      console.log(owned);
      const proposalList = [];
      ...
      this.setState({ owner, owned, wfStatus, voters, proposals: proposalList, nbrProposals, web3, accounts, contract: instance });
    } catch (error) {
      alert(
  ...
  getWinningProposal = async () => {
    ...
    for (let i = 0; i < 5; i++) {
      const id = await contract.methods.winningProposalsID(i).call();
      const prop = await contract.methods.getOneProposal(id).call();
      winningProposals.push(prop);
    }
  }

  getWinningProposals = async () => {
    const { accounts, contract } = this.state;
    const winningList = [];
    for(let i = 0; i < 5; i++) {
      const prop = await contract.methods.winningProposals(i).call({ from: accounts[0]});
      winningList.push(prop);
    }
    console.log("winning proposals", winningList);
  }

  render() {
    if (!this.state.web3) {
      return <div>Loading Web3, accounts, and contract...</div>;
    } else if (this.state.owned && this.state.wfStatus === '0') {
      return (
        <div className="grid-container">
          <div className="header">
            <h2 className="title">Voting</h2>
            <Winner 
              wfStatus={this.state.wfStatus} 
              winningProposal={this.state.winningProposal}             
              />
            <WorkflowStatus wfStatus={this.state.wfStatus} />
          </div>
          <div className="left">
            <Addresse owner={this.state.owner} addr={this.state.accounts[0]} />
            <div>
              <p>
          ...
          {/* <ProposalList proposals={this.state.proposals} contract={this.state.contract} /> */}
          
        </div>
      );
    } else if (this.state.owner === this.state.accounts[0] && this.state.wfStatus === '0') {
      return (
        <div className="App">
          <h2>Voting</h2>
          <br />
