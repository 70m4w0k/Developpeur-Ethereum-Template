...
contract('VOTING', function (accounts) {
    ...
    describe('add proposal', async function () {
        ...
        it('should revert because proposal is empty', async function () {
            await this.VotingInstance.startProposalsRegistering();
            await expectRevert(this.VotingInstance.addProposal("", {from: alice}), "Empty proposal not allowed");
        });

        it('should revert because too many proposals', async function () {
            await this.VotingInstance.startProposalsRegistering();
            for (i = 0; i < 100; i++) {
                this.VotingInstance.addProposal("hello", {from: alice});
            }
            await expectRevert(this.VotingInstance.addProposal("hello", {from: alice}), "Too many propositions already");
        });
